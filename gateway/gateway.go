...
func (gw *Gateway) ignoreTexts(msg *config.Message, input []string) bool {
	for _, entry := range input {
		...
		if err != nil {
			gw.logger.Errorf("incorrect regexp %s for %s", entry, msg.Account)
			continue
		}
		if re.MatchString(msg.Text) {
			gw.logger.Debugf("matching %s. ignoring %s from %s", entry, msg.Text, msg.Account)
			return true
		}
	}
	return false
...
func (gw *Gateway) ignoreNicks(msg *config.Message, input []string) bool {
	...
	for _, entry := range input {
		if msg.Username == entry {
			gw.logger.Debugf("ignoring %s from %s", msg.Username, msg.Account)
			return true
		}
	}
	return false
